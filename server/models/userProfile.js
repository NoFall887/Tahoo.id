...
function insertUser(req, res, next) {
  const queryText =
    "INSERT INTO data_profile(nama, username, password, email, is_admin) VALUES($1,$1, $2, $3, false) RETURNING *;";
...
async function updatePassword(id, password) {
  const queryText = `UPDATE data_profile 
SET password=$2
WHERE id_profile=$1
RETURNING * ;`;
  try {
...
async function updateUser({ id, username, nama, email, foto } = {}) {
  const queryText = `UPDATE data_profile 
SET username = $1,
    nama = $2,
    email = $3,
...
RETURNING * ;`;
  try {
    var res = await pool.query(queryText, [
      username,
      nama,
      email,
      foto,
